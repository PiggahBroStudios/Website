...
gitlog = logging.FileHandler(LOCATION + '/logs/github.log')
gitlog.setLevel(logging.INFO)

app = Flask(__name__)

app.logger.addHandler(handler)
app.logger.addHandler(gitlog)
...
def github_payload():
  ...
  with open(LOCATION + "logs/github/"+str(data['hook']['id'])+".log", 'w') as log:
    log.write("Received JSON: "+json.dumps(data))
    log.close()
  return "Received package id: " +str(data['hook']['id'])
